...
module.exports = {
  config: {
    ...
    cooldowns: 20,
    role: 0,
    shortDescription: "Generate an image using the SDXL model based on a prompt.",
    longDescription: "Generates an image using the SDXL model with the provided prompt.",
    category: "fun",
    guide: "{p}sdxl <prompt>",
  },
  onStart: async function ({ message, args, api, event }) {
    ...
    try {
      const sdxlApiUrl = `https://global-redwans-apis.onrender.com/api/sdxl?prompt=${encodeURIComponent(prompt)}`;
