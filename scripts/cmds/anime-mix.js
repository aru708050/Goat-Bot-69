...
module.exports = {
  config: {
    name: "animemix",
    aliases: ["amix", "animegen", "amx"],
    author: "Redwan",
    version: "2.0",
    cooldowns: 20,
    role: 2,
    shortDescription: "Generate anime-style AI images with Anime Mix.",
    longDescription: "Anime Mix creates stunning, high-quality AI-generated anime-style illustrations based on your prompt.",
    category: "ai",
  ...
  onStart: async function ({ message, args, api, event }) {
    ...
    if (!prompt) {
      return api.sendMessage("❌ | Anime Mix needs a creative prompt! Describe your scene.", event.threadID, event.messageID);
    }
    ...
    api.sendMessage("🌸 | Anime Mix is bringing your vision to life... Please wait!", event.threadID, (err, info) => {
      if (err) return;
...
async function generateImage(prompt, message, api, event, waitMessageID, userId) {
  try {
    const apiUrl = `https://renzsuperb.onrender.com/api/animemix?prompt=${encodeURIComponent(prompt)}`;
    ...
    message.reply({
      body: "🎨 | Your Anime Mix masterpiece is ready! Enjoy your AI-powered anime creation! 🌟",
      attachment: stream,
    });
