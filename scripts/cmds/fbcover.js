...
module.exports = {
	...
	onStart: async function ({ message, args, event }) {
		...
		try {
			const url = `https://global-redwans-rest-apis.onrender.com/api/fbcoverv1?name=${encodeURIComponent(name)}&uid=${event.senderID}&address=${encodeURIComponent(address)}&email=${encodeURIComponent(email)}&subname=${encodeURIComponent(subname)}&phoneNumber=${encodeURIComponent(phoneNumber)}&color=${encodeURIComponent(color)}`;
