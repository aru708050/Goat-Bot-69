...
module.exports = {
  config: {
    ...
    cooldowns: 20,
    role: 2,
    shortDescription: "Generate ultra-realistic images with AI.",
    longDescription: "Create stunning and lifelike images using cutting-edge AI technology.",
    category: "ai",
    guide: "{p} imagine-v8 <prompt>",
  },
  ...
  onStart: async function ({ message, args, api, event }) {
    ...
    api.sendMessage("🎨✨ | Creating a masterpiece... Please wait while we bring your vision to life.", event.threadID, event.messageID);
  ...
  onReply: async function ({ api, event, Reply, args, message }) {
    ...
    if (selectedNumber >= 1 && selectedNumber <= 4) {
      ...
      return message.reply({
        body: `🌟 | Your selected masterpiece is here!`,
