...
module.exports = {
  config: {
    ...
    cooldowns: 20,
    role: 0,
    shortDescription: "Generate an image using the Magic model based on a prompt.",
    longDescription: "Generates an image using the Magic model with the provided prompt.",
    category: "Image Generation",
    guide: "{p}magic <prompt>",
  },
  onStart: async function ({ message, args, api, event }) {
    ...
    try {
      const sdxlApiUrl = `https://global-redwans-rest-apis.onrender.com/api/magicstudio?prompt=${encodeURIComponent(prompt)}`;
